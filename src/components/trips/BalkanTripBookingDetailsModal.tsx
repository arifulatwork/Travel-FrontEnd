...
interface BalkanTripBookingDetailsModalProps {
  bookingId: number;
  onClose: () => void;
}
...
const BalkanTripBookingDetailsModal: React.FC<BalkanTripBookingDetailsModalProps> = ({
  bookingId,
  onClose
}) => {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/auth/balkan-trip/booking/${bookingId}`, {
          headers: {
...
export default BalkanTripBookingDetailsModal;
