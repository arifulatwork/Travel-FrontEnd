...
interface BalkanTripPaymentModalProps {
  clientSecret: string;
  bookingId: number;
  onClose: () => void;
}

const BalkanTripPaymentModal: React.FC<BalkanTripPaymentModalProps> = ({ 
  clientSecret, 
  bookingId, 
  onClose 
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        const response = await fetch(`http://127.0.0.1:8000/api/auth/balkan-trip/payment/confirm`, {
          method: 'POST',
          headers: {
...
export default BalkanTripPaymentModal;