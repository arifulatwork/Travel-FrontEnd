...
interface MontenegroTripPaymentModalProps {
  clientSecret: string;
  bookingId: number;
  onClose: () => void;
}

const MontenegroTripPaymentModal: React.FC<MontenegroTripPaymentModalProps> = ({ 
  clientSecret, 
  bookingId, 
  onClose 
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        const response = await fetch(`http://127.0.0.1:8000/api/auth/montenegro-trip/payment/confirm`, {
          method: 'POST',
          headers: {
...
export default MontenegroTripPaymentModal;