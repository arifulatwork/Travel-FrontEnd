...
interface MontenegroTripBookingDetailsModalProps {
  bookingId: number;
  onClose: () => void;
}
...
const MontenegroTripBookingDetailsModal: React.FC<MontenegroTripBookingDetailsModalProps> = ({
  bookingId,
  onClose
}) => {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/auth/montenegro-trip/booking/${bookingId}`, {
          headers: {
...
export default MontenegroTripBookingDetailsModal;
