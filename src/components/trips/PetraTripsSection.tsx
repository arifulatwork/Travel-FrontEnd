...
import PetraTripPaymentModal from './petra/PetraTripPaymentModal';
import PetraTripBookingDetailsModal from './petra/PetraTripBookingDetailsModal';
...
const PetraTripsSection: React.FC = () => {
  const [trips, setTrips] = useState<PetraTrip[]>([]);
  const [selectedTrip, setSelectedTrip] = useState<PetraTrip | null>(null);
  ...
  const fetchBookings = async () => {
    try {
      const res = await fetch(`${BASE_URL}/api/auth/petra-trip/my-bookings`, {
        headers: {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/api/petra-trips`)
      .then(res => res.json())
      .then(data => {
        setTrips(data);
        setLoading(false);
      ...
      .catch(err => {
        console.error('Failed to fetch petra trips', err);
        setLoading(false);
      });
  ...
  const handleBook = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/auth/petra-trip/book`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`
        },
        body: JSON.stringify({ petra_trip_id: selectedTrip.id })
      });
  ...
  if (loading) {
    return <div className="p-6 text-center">Loading Petra trips...</div>;
  }
